...
for i = 1:ndatasets
   ...
   J1(i) = Jvec(randi(length(Jvec),1)); % randomly generating kappa1 from kappa1vec
    ...
    fakedataEP = EPfakedata_J(alpha(i),J1(i));
    ...
    switch modelfit
        ...
        case {'EP'}
            [perfmodel1_fitted, perfmodel_fitted, alpha_estimate, J1_estimate, temp] = EPmodelfitting_J(fakedataEP)
            alphaest(i) = alpha_estimate;
            J1est(i) = J1_estimate;
        case {'SR'}
            [perfmodel1_fitted, perfmodel_fitted, K_estimate, alpha_estimate, J1_estimate,temp] = SRmodelfitting_J(fakedataEP)
        case {'VP'}
            [perfmodel1_fitted, perfmodel_fitted,alpha_estimate,kappabar_estimate,tau_estimate,temp] = VPmodelfitting_J(fakedataEP)
    end
