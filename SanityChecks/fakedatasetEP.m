...
for i = 1:ndatasets
    ...
    kappa1(i) = kappa1vec(randi(length(kappa1vec),1)); % randomly generating kappa1 from kappa1vec
    alpha(i) =  alphavec(randi(length(alphavec),1));   % randomly generating alpha from alphavec
    
    fakedataEP = EPfakedata(alpha(i),kappa1(i));
    
    modelfit = 'VP';
    switch modelfit
        case {'IL'}
            [perfmodel1_fitted, perfmodel_fitted, eps_estimate ,K_estimate, temp] = ILmodelfitting(fakedataEP)
        case {'EP'}
            [perfmodel1_fitted, perfmodel_fitted, alpha_estimate, kappa1_estimate, temp] = EPmodelfitting(fakedataEP)
            alphaest(i) = alpha_estimate;
            kappa1est(i) = kappa1_estimate;
        case {'SR'}
            [perfmodel1_fitted, perfmodel_fitted, K_estimate, alpha_estimate, kappa1_estimate,temp] = SRmodelfitting(fakedataEP)
        case {'VP'}
            [perfmodel1_fitted, perfmodel_fitted,alpha_estimate,kappabar_estimate,tau_estimate,temp] = VPmodelfitting(fakedataEP)
    end
...
figure;
scatter(kappa1, kappa1est)
xlabel('\kappa1'); ylabel('\kappa1');
